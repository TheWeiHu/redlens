# Defined before the imports below: submodules (e.g. arctic's User-Agent)
# read it while the package is still initializing.
__version__ = "0.2.0"  # keep in sync with pyproject.toml

from redlens.analytics import compute_user_analytics  # noqa: E402
from redlens.db import connect, init_schema  # noqa: E402
from redlens.errors import NotFound, RedlensError  # noqa: E402
from redlens.ingest import sync_user  # noqa: E402
from redlens.models import Comment, Post, User, UserAnalytics  # noqa: E402

__all__ = [
    "Comment", "NotFound", "RedlensError", "Post", "User", "UserAnalytics",
    "__version__", "compute_user_analytics", "connect", "init_schema", "sync_user",
]
